...
export function WidgetCard({ widget, isEditMode, isPublic = false, onDelete, children, compact = false }: WidgetCardProps) {
  ...
  const getDisplayTitle = (title: string): string => {
    ...
    // 컴팩트 모드일 때 긴 타이틀 축약
    if (computedIsCompact && title.length > 6) {
  ...
  return (
    ...
    >
      ...
      {/* 컨텐츠 - Stealth 규격: padding 12px (기본, 20% 축소), compact 모드에서는 p-2 (8px) */}
      <div 
        className={computedIsCompact ? 'p-2 min-h-[64px]' : 'py-1.5 min-h-[64px]'}
        style={computedIsCompact ? {} : {
          paddingLeft: 'var(--stealth-spacing-card)',
          paddingRight: 'var(--stealth-spacing-card)',
        }}
      >
