...
interface FavoritesSectionProps {
  ...
  onShowGuide?: () => void;          // 가이드 다시보기
  onSaveData?: () => void;           // 저장
  isLoggedIn?: boolean;              // 로그인 여부
  onRequestLogin?: () => void;       // 로그인 요청
}
...
function SimpleWebsite({
  ...
  let website =
    websites.find((w) => w.id === websiteId) || undefined;
  ...
  return (
    <div
      className={`sfu-favorite-item relative p-1 rounded shadow-sm border transition-all h-6 group ${
        isDraggingOver ? 'sfu-drop-zone' : ''
      }`}
      draggable
...
export function FavoritesSectionNew({
  ...
  const handleDrop = (e: React.DragEvent, targetId: string | null) => {
    ...
    const categoryWebsiteId = e.dataTransfer.getData('text/plain'); // 카테고리에서 온 경우
    const favoriteWebsiteId = e.dataTransfer.getData('websiteId');  // 즐겨찾기 내부 이동
    ...
    if (favoriteWebsiteId) {
      ...
      // targetId가 아이템 id일 수도 있어 단순 push
      newDest.push(moved);
    ...
    <div className="max-w-screen-2xl mx-auto px-5 sm:px-2 py-8 transition-colors duration-300">
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex gap-2">
          ...
          {/* ✅ 가이드 보기 버튼 (App.tsx의 onShowGuide와 연결됨) */}
          <button
