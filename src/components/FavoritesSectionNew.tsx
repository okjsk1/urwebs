...
function SimpleFolder({
  ...
  return (
    ...
    >
      <div className="controls w-full flex items-center gap-2 mb-2">
        ...
        {isEditing ? (
          ...
          <div className="flex items-center gap-2 flex-1 group">
            <h3 className="title text-gray-800 flex-1 group-hover:text-blue-600 dark:text-gray-200 dark:group-hover:text-blue-400">
              {folder.name}
            </h3>
            <button
              onClick={() => {
      ...
      <div className="w-full flex flex-col gap-1 flex-1 overflow-y-auto min-h-0">{children}</div>
    </div>
  );
}
